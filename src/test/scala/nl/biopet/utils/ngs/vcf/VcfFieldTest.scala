...
class VcfFieldTest extends  BiopetTest {
  ...
  def testInCorrect(): Unit = {
    ...
    }.getMessage should startWith("requirement failed: A field should be formatted like: <tag>:<method>. Possible methods: ")

    intercept[IllegalArgumentException] {
      VcfField.fromArg("DP:Something_else")
    }.getMessage should startWith("Method 'Something_else' does not exis. Possible methods: ")
  }
}